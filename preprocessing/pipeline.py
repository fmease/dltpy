...
def run_pipeline(projects: list) -> None:
    ...
    ParallelExecutor(n_jobs=args.jobs)(total=len(projects))(
        delayed(process_project)(i, project)
        for i, project in enumerate(projects, start=args.start)
    )
...
def process_project(i, project):
    try:
        ...
        filtered_project_directory = project_filter.filter_directory(
            raw_project_directory
        )
